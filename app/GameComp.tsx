...
import LockedComp from './LockedComp'
import { useAccount, usePublicClient, useWriteContract } from 'wagmi'
import { formatEther, parseEther } from 'viem'

const GameComp = ({goBack}: {goBack: () => void}) => {
    ...
    // State for showing locked component and modal
    ...
    // Function to check ASTR balance - moved up before any conditional returns
    ...
    useEffect(() => {
        // Convert input to number, multiply by 100, and format
        const numericInput = parseFloat(inputValue) || 0;
        const calculated = (numericInput * 100).toFixed(0);
        setOutputValue(calculated);
    }, [inputValue]);
    
    // Check balance when address changes
    useEffect(() => {
        if (address) {
    ...
    // Update the input handling and calculation
    ...
    // Check staked balance when address changes
    useEffect(() => {
        getPoints();
        checkStakedBalance();
    }, [address]);
    
    // Update the handleLockASTR function
        ...
        try {
            ...
            if (stakeHash) {
                try {
                    ...
                    try {
                        const pointsToAward = Math.floor(dollarAmount * 100); // Convert ASTR amount to points
    ...
    // Handle back from locked screen

    
    // If showing the locked screen, render LockedComp
    if (showLockedScreen) {
        return <LockedComp  />;
    }
            ...
            <div className="flex flex-col items-center w-full h-full py-8 px-4 rounded-3xl overflow-hidden" style={{
                ...
                <div className="text-center mb-6">
                    <p style={{ 
                      ...
                      Lock ASTR for Coins! Unlock<br />after rounds!
                    </p>
                </div>
                ...
                <div className="text-center mb-8 mt-2 pt-4">
                    <p style={{ 
                      ...
                      Need $10 ASTR to start. Add<br />more to win big!
                    </p>
                </div>
                ...
                <div className="flex flex-col w-full max-w-md gap-3 px-4">
                    ...
                    <button
                        onClick={goBack}
                        type="button"
                        className="w-full flex justify-center items-center text-gray-400 text-xl font-bold shadow-lg transition-all hover:shadow-xl"
                        style={{ 
                            fontFamily: 'Arial, sans-serif',
                            background: 'linear-gradient(90deg, #F7CBBF 0%, #CDAFFA 100%)',
                            border: '1.5px solid #FFF',
