...
const MONGODB_URISNAKE = "mongodb+srv://devultimate:Msdthiru7!@cluster0.nt80u.mongodb.net/snake" ;
...
export async function connectToDatabaseSnake() {
  const cached = globalWithMongo.mongooseCache!;
  ...
  if (!cached.promise) {
    ...
    cached.promise = mongoose.connect(MONGODB_URISNAKE, opts)
      .then((mongoose) => {
        console.log('Connected to MongoDB');
        return mongoose;
      })
